"""Bring `runs/nce1_masked/` home, put the analysis files on GitHub, then destroy the volumes.

    python modal_nce1_masked_finish.py             # fetch -> verify -> commit+push -> delete
    python modal_nce1_masked_finish.py --no-delete # everything except the delete
    python modal_nce1_masked_finish.py --verify    # verify what is already local, touch nothing
...
def write_result_md(dest: Path, summary: dict) -> None:
    ...
    lines = [
        ...
        "so this is a ONE-delta comparison against it and the delta is the sibling-late mask.",
        "",
